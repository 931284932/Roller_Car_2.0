...
{
	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_8,(GPIO_PinState)BitValue);//根据BitValue，设置SS引脚的电平
}
...
{
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_10,(GPIO_PinState)BitValue);		//根据BitValue，设置SCK引脚的电平
}
...
{
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, (GPIO_PinState)BitValue);		//根据BitValue，设置MOSI引脚的电平，BitValue要实现非0即1的特性
}
...
{
	return HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_11);			//读取MISO电平并返回
}
...
void SPI_Init(void)
{
	...
	HAL_GPIO_Init(GPIOC, &GPIO_InitStructure);					//将PC10引脚初始化为推挽输出
	
	GPIO_InitStructure.Mode     = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStructure.Pin      = GPIO_PIN_15 | GPIO_PIN_8;
	GPIO_InitStructure.Speed    = GPIO_SPEED_FREQ_HIGH;
	HAL_GPIO_Init(GPIOA, &GPIO_InitStructure);					//将PA15和PA8引脚初始化为推挽输出
	
	GPIO_InitStructure.Mode 		= GPIO_MODE_INPUT;
	GPIO_InitStructure.Pull     = GPIO_PULLUP;
	GPIO_InitStructure.Pin      = GPIO_PIN_11;
	GPIO_InitStructure.Speed    = GPIO_SPEED_FREQ_HIGH;
	HAL_GPIO_Init(GPIOC, &GPIO_InitStructure);					//将PA8引脚初始化为上拉输入
